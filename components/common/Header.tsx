...
const Header = () => {
  ...
  return (
    <header className="bg-transparent z-50 tracking-widest w-full">
      <div
        className={`flex items-center justify-between px-4 md:px-15 py-4 md:py-6 w-full transition-colors duration-300 ${isShop || isContact || isCart || isCheckout || isProductDetail || isOrders
          ? "text-black"
          : "text-[var(--nav-color)]"
          }`}>

        <button onClick={toggleMenu} aria-label="Menu" className="flex gap-3 items-center text-xl">
          <FaBars className="w-6 h-6 cursor-pointer" /> Menu
        </button>
      ...
      <div
        className={`fixed top-0 left-0 h-full w-64 bg-white z-[999] shadow-lg transform transition-transform duration-300 ease-in-out ${isMenuOpen ? "translate-x-0" : "-translate-x-full"
          }`}>
